...
Traditional quantitative trait locus (QTL) analyses have focused on discovering ``mean QTL'' (mQTL), regions of the genome where allelic variation drives heterogeneity of phenotype mean, while assuming that the residual variance, that is, the instrinsic stability or noisiness of the phenotype, is identical for every individual in the mapping population.
It has long been recognized, however, that the residual variance is itself heritable \WV{\citep{Falconer1965,Lynch1998}}{Do they definitely talk about this?}.
This phenomenon has been described theoretically \citep{Hill2004-uo,Hill2010}, demonstrated in inbred model organisms \WV{\citep{Sorensen2015}}{Seems a bit recent} and crops \citep{Yang2012-aw}, and exploited in livestock improvement efforts \citep{Mulder2008,Ibanez-Escriche2008-ie}.
%To help dissect the genetic basis, 
Correspondingly, several groups have proposed statistical methods for mapping QTL controling the extent of this residual variance, these sometimes termed ``variance QTL'' (vQTL) \citep{Pare2010,Ronnegard2011a,Ronnegard2012,Cao2014}\WV{\citep{Soave2017,Dumitrascu2018}}{add Englehardt, Soave?}.

% Traditional quantitative trait locus (QTL) analyses have focused on discovering ``mean QTL'', regions of the genome where allelic variation drives heterogeneity of phenotype mean, while assuming that the residual variance, that is, the intrinsic stability or noisiness of the phenotype, is identical for every individual in the mapping population.
% However, it has long been recognized that the extent of residual variation is itself a heritable trait \citep{Falconer1965,Lynch1998}.
...
% Recent work, has begun to recognize heterogeneous residual variance in mapping efforts by identifying an additional type of QTL---those that influence the extent of residual variance, sometimes termed ``variance QTL'' (vQTL) \citep{Pare2010,Ronnegard2011a,Ronnegard2012,Cao2014}. 
...
%
%In two companion articles, we characterize the method and competitors in the setting where variance heterogeneity is driven by a background factor (\CortyMethodsPaper),  and use it to discover new QTL from two existing data resources (\CortyReanalysisPaper).
%
%The R package \texttt{vqtl} implements mean-variance QTL mapping
...
%We introduce the \texttt{vqtl} package as follows.
...
% \section*{Old intro material -- TO DELETE OR REPURPOSE}

% Quantitative Trait Locus (QTL) mapping studies in experimental crosses have provided important insights on nearly every trait of interest in human health, as well as livestock and crop improvement programs.
