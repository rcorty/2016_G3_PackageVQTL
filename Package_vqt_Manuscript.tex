...
On each phenotype we then conduct a genome scan using \WV{standard approximations to interval mapping \citep{Lander1989a}}{Surely HK and MC? We don't actually use the Lander mixture model.}, and the mean-variance approach to QTL mapping, which includes a test for mQTL, a test for vQTL, and a test for mvQTL.
...
Analogously, the central function for in package \texttt{vqtl} is \WV{\texttt{scanonevar}}{Did Karl originally implement this? If so, we should say something about it.}.
