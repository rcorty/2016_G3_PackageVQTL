...
\lhead{Manuscript ID\\G3/2018/200159}
...
\Response{
  The editor suggested references that relate to the heritabiliyty of environmental variance.
  This literature was foundational toward the development of the work presented in this manuscript, so we agree with the editor that it should be cited in the introduction.
...
\Response{
  To most clearly illustrate the standard workflow associated with package \texttt{vqtl}, the authors have demonstrated its usage on a simulated cross with an unrealistically-small genome.
  The authors are encouraged by the reviewer's enthusiasm to see the package applied to real datasets.
  This manuscript has been submitted as part of a three-paper companionship, where each of the other papers contains an analysis of real data using package \texttt{vqtl}.
  Thus, the authors contend it is appropriate to maintain this paper with only limited reference to real data --- a real dataset is used in in the first performance benchmark, as described below.
}
